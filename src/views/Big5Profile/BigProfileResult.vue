<template>
    <ion-page>
        <ion-content>
            ...
            <ion-card class="gray-card ion-padding-start ion-padding-end">
                <ion-card-header>
                    <ion-card-title>Big 5 Profile:</ion-card-title>
                </ion-card-header>
                <ion-card-content>
                    Lorem ipsum dolor sit amet consectetur. Faucibus urna facilisis tellus donec bibendum velit. Non
                    tincidunt nulla ultrices lobortis.
                </ion-card-content>
            </ion-card>
            <ion-grid>
                <ion-row>
  ...
  <div v-for="trait in traits" :key="trait.name">
    ...
    <div class="custom-class">
      <ion-label :class="['ion-text-start', trait.lowEndColorClass]">{{ trait.lowEnd }}</ion-label>
      <ion-label :class="['ion-text-end', trait.highEndColorClass]">{{ trait.highEnd }}</ion-label>
    </div>
    <ion-range @ionChange="e => onRangeChange(e.detail.value, trait.name)"></ion-range>
  </div>
</ion-col>

                </ion-row>
            </ion-grid>
            <ion-button expand="block" fill="solid" class="ion-padding">RETAKE TEST</ion-button>
            <ion-button expand="block" fill="outline" class="ion-padding top-content">GO BACK TO HOMEPAGE</ion-button>
        </ion-content>
    </ion-page>
</template>

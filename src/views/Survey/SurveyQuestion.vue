<template>
  ...
  <ion-content>
    <ProgressBar :tempIndex="tempIndex + 1" :data="statusStore?.questionnaire?.size + 1" :showLabel="true" />
    <div v-if="tempIndex <= statusStore?.questionnaire?.size - 1">
      ...
      <ion-list>
        <ion-radio-group v-model="selectQuestion" @ionChange="
      setQuestionActive(
        'description_low',
        'description_midlow',
        'description_medium',
      ...
      'radio-item-selected':
        ...
        statusStore?.questionnaire?.get(currentIndex)?.question
          ?.description_low,
    }" lines="full" class="radio-item">
            <!-- <ion-label>{{ statusStore?.questionnaire?.get(currentIndex)?.question?.description_low }}</ion-label> -->
            <ion-radio :value="statusStore?.questionnaire?.get(currentIndex)?.question
      ?.description_low
      ...
      statusStore?.questionnaire?.get(currentIndex)?.question
        ...
        'radio-item-selected':
          ...
          statusStore?.questionnaire?.get(currentIndex)?.question
            ...
            <ion-radio :value="statusStore?.questionnaire?.get(currentIndex)?.question
      ?.description_medium
      " justify="start" label-placement="end">{{
      statusStore?.questionnaire?.get(currentIndex)?.question
        ?.description_medium
    }}</ion-radio>
      ...
      <div>
        ...
        <div v-for="item2 in statusStore?.questionnaire?.get(currentIndex)
      ?.question?.options" :key="item2.id" class="topMargin">
      ...
      ">
              ...
              <ion-checkbox :value="item2.id + '_' + item2.favourable"
                :checked="selected[item2.id + '_' + item2.favourable]"></ion-checkbox>
      ...
      ">
              <ion-checkbox :value="item2.id + '_' + item2.unfavourable"
                :checked="selected[item2.id + '_' + item2.unfavourable]"></ion-checkbox>
    ...
    <ion-footer>
      <div class="button-container">
        <ion-button class="previousButton" @click="processBackItem" :disabled="tempIndex <= 0">Previous</ion-button>
        <ion-button v-if="tempIndex <= statusStore?.questionnaire?.size - 1" @click="processNextItem"
          :disabled="tempIndex >= statusStore?.questionnaire?.size" class="nextButton">Next</ion-button>

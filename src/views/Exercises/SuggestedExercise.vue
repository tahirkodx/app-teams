<template>
    <ion-page>
      ...
      <ion-content >
        <div class="header-container">
          ...
          <h1 class="ion-padding">{{playbookStore?.suggestedExercise?.title}}</h1>
          <h2>
            {{playbookStore?.suggestedExercise?.description}}
          </h2>
          
          <!-- <LinkButton label="Ask Help" expend="block" link="teamsupport" /> -->
          <ion-button v-if="!showHideOwner" class="custom" @click="showHideFun" expand="block">ADD EXERCISE</ion-button>
        </div>
  
        <div v-if="!showHideOwner" class="tab-container">
          <div class="tabs">
            <div
              class="tab"
        ...
        <EditOwnerStatus v-else :update="showHideFun" />
        <!-- ... more content ... -->
      </ion-content>
    </ion-page>
  </template>
  ...
  import EditOwnerStatus from "@/components/Exercise/EditOwnerStatus.vue";
  import { usePlaybookStore, useUserStore, useTeamStore } from "@/store";
  const userStore = useUserStore();
  const playbookStore = usePlaybookStore();
  const loading = ref(false);
  const showHideOwner = ref(false);
  ...
  const showHideFun = () => {
    console.log(showHideOwner.value);
    showHideOwner.value = !showHideOwner.value;
    console.log(showHideOwner.value);
  };
  ...
  onMounted(async () => {
    await Promise.all([
      playbookStore.getSuggestedExercise(userStore.teamID, exerciseID),

...
const useUserStore = defineStore('user', () => {
  ...
  async function getUserOrganization() {
    ...
    console.log("User organization:", organization.value);
  }
  ...
  const teamID = computed<TTeamID>(() => {
     const lastTeamUsed = settings.value?.last_team_used ? settings.value.last_team_used : null;
    
    if (lastTeamUsed) {
    return lastTeamUsed;
  } 
  // else if (teamStore.teams.value.size > 0) {
  //   // setNextActiveTeam() - You might want to implement this function
  // } 
  else {
    return '';
  }

...
const usePlaybookStore = defineStore("Playbook", () => {
  ...
  const suggestedExercise = ref<any>();
  const exercises: any = ref(null);
  const suggestedExercises: any = ref(null);
  ...
  async function getSuggestedExercises() {
    console.log("Here is Adil", userStore.teamID)
    if (suggestedExercises.value === null) {
    suggestedExercises.value = await PlaybookAPI.getSuggestedExercises();
    }
  ...
  const filteredExercises = computed(() => {
    console.log("Here is team id ", userStore.teamID)
    return (filter: any) => {
  ...
  async function getSuggestedExercise(teamID: TTeamID, exerciseID: string) {
    // todo need to change this prams
  
    const payload = {
      teamId: "1d53775de8a9421aa2f1327cb4d3d87e",
      exerciseID: exerciseID,
    };
    suggestedExercise.value = await PlaybookAPI.getSuggestedExercise(payload);
    // if (exercises.value.has(teamID)) {
    //   const teamExercises = exercises.value.get(teamID);
    //   const arrayFromTeamExercises = Array.from(teamExercises);
    //   console.log(arrayFromTeamExercises)

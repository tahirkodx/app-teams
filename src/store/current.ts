...
export async function fetchCurrent(force: boolean = false){
    Promise.all([
        teamStore.teams.getTeams(force),
        userStore.settings.getUserSettings(force),
    ])
}